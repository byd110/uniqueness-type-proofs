...
Inductive step: stmt -> store -> heap -> class_table -> store -> heap -> Prop :=
 ...
 | step_mcallU: forall σ h ct x y m z c d c' o fl init ml Tp s t σ' h' ts r xt,
   ...
   x < length σ ->
   z < length σ ->
   step (smcall $x $y m $z) σ h ct (update (update σ' xt (TCls c' TSBot, r)) x (TCls c' TSUnique, r)) h'  
...
Proof.
   ...
   1,2: rewrite <- update_length; auto.
...
Proof.
  ...
  - inversion H'; subst. 1,3: rewrite H10 in H; inversion H; subst; erewrite H11 in H0; 
    ...
    specialize (teval_deterministic H3 H19) as H22; subst. reflexivity.
  (* smcallU *)
  - inversion H'; subst. 1,2: rewrite H12 in H; inversion H; subst; erewrite H13 in H0; 
    inversion H0; subst; rewrite H14 in H1; inversion H1; subst.  
    rewrite H12 in H; inversion H; subst.
    rewrite H13 in H0; inversion H0; subst. rewrite H14 in H1; inversion H1; subst.
    rewrite H17 in H4; inversion H4; subst. specialize (IHstep σ'0 Hh). intuition; subst. 
    specialize (teval_deterministic H3 H16) as H26; subst. reflexivity.
  (* slettmC *)
